<?php
/**
 * The class for WP-CLI commands for Theme Freesia Demo Import plugin.
 *
 * @package ocdi
 */
...
class WPCLICommands extends \WP_CLI_Command {
	...
	private function import_predefined( $predefined_index ) {
		if ( ! is_numeric( $predefined_index ) ) {
			WP_CLI::error( esc_html__( 'The "predefined" parameter should be a number (an index of the OCDI predefined demo import)!', 'theme-freesia-demo-import' ) );
		}
	...
	private function import_content( $relative_file_path ) {
		...
		Helpers::append_to_file( '', $this->ocdi->log_file_path, esc_html__( 'Importing content' , 'theme-freesia-demo-import' ) );
