<?php
/**
 * Main Theme Freesia Demo Import plugin class/file.
 *
 * @package ocdi
 */
...
/**
 * Theme Freesia Demo Import class, so we don't have to worry about namespaces.
 */
class OneClickDemoImport {
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'            => array(
						...
						'installing'               => esc_html__( 'Installing...', 'theme-freesia-demo-import' ),
						'importing'                => esc_html__( 'Importing...', 'theme-freesia-demo-import' ),
						'successful_import'        => esc_html__( 'Successfully Imported!', 'theme-freesia-demo-import' ),
						'install_plugin'           => esc_html__( 'Install Plugin', 'theme-freesia-demo-import' ),
						'installed'                => esc_html__( 'Installed', 'theme-freesia-demo-import' ),
						'import_failed'            => esc_html__( 'Import Failed', 'theme-freesia-demo-import' ),
						'import_failed_subtitle'   => esc_html__( 'Whoops, there was a problem importing your content.', 'theme-freesia-demo-import' ),
						'plugin_install_failed'    => esc_html__( 'Looks like some of the plugins failed to install. Please try again. If this issue persists, please manually install the failing plugins and come back to this step to import the theme demo data.', 'theme-freesia-demo-import' ),
						'content_filetype_warn'    => esc_html__( 'Invalid file type detected! Please select an XML file for the Content Import.', 'theme-freesia-demo-import' ),
						'widgets_filetype_warn'    => esc_html__( 'Invalid file type detected! Please select a JSON or WIE file for the Widgets Import.', 'theme-freesia-demo-import' ),
						'customizer_filetype_warn' => esc_html__( 'Invalid file type detected! Please select a DAT file for the Customizer Import.', 'theme-freesia-demo-import' ),
						'redux_filetype_warn'      => esc_html__( 'Invalid file type detected! Please select a JSON file for the Redux Import.', 'theme-freesia-demo-import' ),
					),
				)
			);
	...
	public function upload_manual_import_files_callback() {
		...
		if ( empty( $_FILES ) ) {
			wp_send_json_error( esc_html__( 'Manual import files are missing! Please select the import files and try again.', 'theme-freesia-demo-import' ) );
		}
	...
	private function final_response() {
		...
		$response['title'] = esc_html__( 'Import Complete!', 'theme-freesia-demo-import' );
		$response['subtitle'] = '<p>' . esc_html__( 'Congrats, your demo was imported successfully. You can now begin editing your site.', 'theme-freesia-demo-import' ) . '</p>';
		$response['message'] = '<img class="ocdi-imported-content-imported ocdi-imported-content-imported--success" src="' . esc_url( OCDI_URL . 'assets/images/success.svg' ) . '" alt="' . esc_attr__( 'Successful Import', 'theme-freesia-demo-import' ) . '">';

		if ( ! empty( $this->frontend_error_messages ) ) {
			$response['subtitle'] = '<p>' . esc_html__( 'Your import completed, but some things may not have imported properly.', 'theme-freesia-demo-import' ) . '</p>';
			$response['subtitle'] .= sprintf(
				wp_kses(
				/* translators: %s - link to the log file. */
					__( '<p><a href="%s" target="_blank">View error log</a> for more information.</p>', 'theme-freesia-demo-import' ),
					array(
						'p'      => [],
						'a'      => [
							'href'   => [],
	...
	public function redirect_from_old_default_admin_page() {
		...
		if ( $pagenow == 'themes.php' && isset( $_GET['page'] ) && $_GET['page'] == 'pt-theme-freesia-demo-import' ) {
